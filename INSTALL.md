...
Optional: set `CEREBRAS_API_KEY` in the same ignored `.env.local` to let Jev ask Qwen to draft text for fields and suggest recovery after repeated action errors. The default `CEREBRAS_MODEL` is `qwen-3.8-27b`. `GROQ_API_KEY` remains supported as an alternative; Cerebras takes precedence if both are set. The user can omit both and still use exact `inputs` and all ordinary computer actions. `computer_health` reports the active helper.
